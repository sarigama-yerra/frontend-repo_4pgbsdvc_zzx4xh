...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-orange-50 via-white to-white text-gray-900">
      ...
      {page === 'home' && (
        <main>
          <Hero onExplore={() => document.getElementById('categories')?.scrollIntoView({ behavior: 'smooth' })} />
          ...
          <footer className="mt-12 border-t border-orange-100/60 bg-white/60 backdrop-blur">
            <div className="max-w-6xl mx-auto px-4 py-6 text-sm text-zinc-600 flex items-center justify-between">
              <span>© {new Date().getFullYear()} FlameLunch — доставка бизнес-ланчей</span>
              <button onClick={() => setPage('cart')} className="px-4 py-2 rounded-xl bg-zinc-900 text-white">Перейти в корзину</button>
            </div>
          </footer>
        </main>
      )}
